...
autoplot(varespec_pca, display="sites", legend.position="none", geom="text") 

# Plot PC1 and PC2 of the species
autoplot(varespec_pca, display="species", legend.position="none", arrows=FALSE) 
```
...
I have added `geom="text"` to force display as text rather than simply points.
...
axes. To undertake a correspondence analysis, use the `cca()` function instead of
the `rda()` function. When `cca()` is called without any explanatory variables it
defaults to correspondence analysis.
...
autoplot(varespec_ca, display="sites", legend.position="none", geom="text") 

# Plot CA1 and CA2 of the species
autoplot(varespec_ca, display="species", legend.position="none", arrows=FALSE, geom="text") 
```
...
* in the `autoplot` code, to obtain separate sites and species plots, replace
`display="sites"` with `layers="sites"`.
...
autoplot(varespec_nmds, layers="sites", legend.position = "none", geom="text")
autoplot(varespec_nmds, layers="species", legend.position = "none", geom="text")
```

NMDS is considered to be the most robust unconstrained ordination method for
many biologists. See the paper by Minchin (1987) <a href="www/Minchin.pdf" target="_blank">here</a>  The `metaMDS()` function
combines all of Minchin's recommendations into a single command.
