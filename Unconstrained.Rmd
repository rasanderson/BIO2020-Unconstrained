...
Typically in ecology, environmental science and marine biology you have one response
variable, with one or more explanatory variables. Thus, you might be interested
in the effects of nutrient runoff from land on the growth of marine macro-algae.
Your response variable is the cover of macro-algae, and your explanatory the 
nutrient runoff. You might be interested in the effects of hedgerow management
on the numbers of nesting song-thrushes. Your response would be "number of nests"
and your explanatory could be "number of cuts per year". In both these examples
you could have additional explanatory variables. For example the depth of water
will also affect the % cover of macro-algae. Which species of plant comprise the
hedgerow (hawthorn, blackthorn, privet etc.) will also affect birds. The key
point is that in all these examples you only have **one** response variable.

However, there can be numerous examples where you collect lots of response
variables. This will often give you response data in the form of a table.
Examples include:

* plant species x vegetation quadrats
* invertebrate species x pitfall traps
* bird species x transect number
...
the attributes are species, whilst the samples are quadrats, pitfall traps,
transect IDs, marine sample code etc. Note that with modern DNA sequencing
techniques, these tables now apply to genomics data, which are being used more
widely in biodiversity and environmental studies. Sometimes for some species,
such as bacteria and fungi, taxonomic names may not be known, and you will see
them referred to as "operational taxonomic units" or OTUs, but the principles
remain unchanged.

These tables can be large, for example a typical invertebrate survey can easily
record 50 or 100 species within one family, whilst genetic studies can produce hundreds of
...
* Which quadrats have few plant species in common?
* Which woodlands have the most similar bird communities?
* Which mammals are rarely found together?
* Which species of butterflies and plants co-occur?
...
### Entering your data and reading it into R
As your data are typically going to be in the form of a table, you will often
find it easiest to enter your raw data into Microsoft Excel, with column names
being your species, and row names your samples. However, this does **not** match
the generally recommended **tidyverse** methods of storing data. Indeed, an R
`tibble` does not have rownames, which will cause many multivariate methods to
fail. Fortunately, you can use the `rownames_to_column()` and `column_to_rownames()`
functions to account for all circumstances, and you will practice this later. If
you have originally entered your data in "non-tidy" format, i.e. a rectangular
table in Excel, with each row a sample, and each column a species, you will 
probably find it easier to use the `read.csv()` function rather than the 
`read_csv()` function. The latter creates a `tibble` by default, which does not
contain the rownames that are needed for multivariate analyses.
...
get a summary of the data. **Note** as it is a conventional R `data.frame` rather
than a `tidyverse` structured `tibble`, the `dune` dataset has rownames.
How many attributes (species) are present? How many samples (quadrats)? What
seems to be a common number in the data? What are the column names? What are
the rownames?
...
ourselves to three, that you might encounter in your MSc research projects.
...
explanatory variables it automatically uses constrained analysis. Remember
that in both constrained and unconstrained analyses the "response variable" is
actually your `data.frame` containing lots of variables, typically spp x sites. The `summary()` function
...
display the results using the default `plot()` command. Labels can sometimes be
a little cluttered, so you find it easier to export to Excel for plotting some
data, or using the development package `ggvegan`. As development packages are
not on the main R CRAN repository they are harder to install, but you may wish
to try it in your own time.

The first plot beelow displays quadrats (sites), and the second species. You will
often find it easiest to create a blank plotting "canvas" before adding the text
with the quadrat numbers or species codes. The `cex` option controls the font
size: set it to a low value if you have a lot of text to display.

```{r plot_dune, echo=TRUE}
# Display site scores
plot(dune_pca, type = "n")  # set up a blank canvas
text(dune_pca, display="sites", cex = 0.7)

# Display quadrat PCA scores for axes 1 and 2
plot(dune_pca, type = "n") 
text(dune_pca, display="species", cex = 0.7)
...
**Note**
When using RStudio yourself, you can interactively add
the labels for the quadrats or species via the `identify()` function.
...
# Create boxplot
ggplot(aes(y = dune_pc1, x = Moisture), data = dune.env) +
  geom_boxplot()
```
...
horizon is continuous, therefore `geom_point()` is a more appropriate function
than `geom_boxplot()`.

**Note** If you are familiar with `ggformula` (part of the `mosaic` package) e.g.
from BIO8075 Systematic Review and Meta-analysis, feel free to use the `gf_` 
functions such as `gf_boxplot()` or `gf_point()` if you prefer.
...
# ggplot2 structure
ggplot(aes(y = dune_pc1, x = A1), data = dune.env) +
  geom_point()
ggplot(aes(y = dune_pc1, x = Management), data = dune.env) +
  geom_boxplot()
ggplot(aes(y = dune_pc1, x = Use), data = dune.env) +
  geom_boxplot()
ggplot(aes(y = dune_pc1, x = Manure), data = dune.env) +
  geom_boxplot()

# Equivalent plots using ggformula package which you may find easier
# library(ggformula)
...
The `vegan` package's `metaNMDS()` function combines
all of Minchin's recommendations into a single command.

## Summary
Unconstrained ordination methods are designed to find the relationships between
the columns (typically species) and rows (typically sites or quadrats) in your
multivariate data. When you have data collected for multiple species it is
impractical to analyse each one separately, and even if you had the time, the
results would be misleading and over-complicated. Ordination provides a simple
way to visualise the complex data. The main types are:

* Linear methods such as PCA. Short environmental gradients
* Unimodal methods such as CA. Long environmental gradients
* NMDS. Rank-based method, particularly useful where species distributions in
raw data are very skewed

The main challenge with all three methods is interpreting what the ordination
axes represent in a biological context. Relate them back to your environment, or
knowledge about the life-history or habitats of your species or samples to help
you interpret them.