...
usually very cluttered and difficult to read. The `nes8010.R` script contains an
...
plot, by adding `+` to the end of a line with the usual range of `ggplot2`
functions that you have already used. This can be useful if you want to set
titles, or axis limits.

Setting axis limits can sometimes make it easier to relate species and samples;
this is shown in the example below. The first plot displays quadrats and species
on the same graph, but this can be cluttered and hard to interpret. The next two
graphs show them separately. By default `ggplot2` will autoscale the x- and y-axes
but to compare the plots it is useful to set the limits manually.
...
the `gg_title()` function, and also set the x- and y-axis limits via `xlim()` and
`ylim()` functions to ensure they are on the same scale. This last step is
optional: sometimes it is useful to omit. By default, as we have numerous 
quadrats, these will show as points. I've added `geom="text"` to ensure quadrat
numbers and species names are displayed (other option is to display as
`geom="point"` which R defaults to once there are too many quadrats or species).
...
# ggformula structure; in an R session requires you to load ggformula library
...
many biologists. See the paper by Minchin (1987) <a href="www/Minchin.pdf" target="_blank">here</a>  The `ordi_nmds()` function in the `nes8010.R` script is
