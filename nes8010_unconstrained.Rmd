...
three lines to activate the `vegan`, `ggplot2` and `nes8010` commands. The
`nes8010.R` file contains various functions to make it easier to use R for
multi-species analysis, and produce `ggplot2`-compatible graphs.
...
you can use the `scores()` function, with `choices=1` for just the first axis

```{r extract dune PC1}
dune_pc1 <- scores(dune.pca, display="sites", choices=1)
```

This can be compared with some of your explanatory data, for example:

```{r ggplot boxplot of dune pc1}
ggplot(dune.env, aes(x=Moisture, y=dune_pc1)) +
   geom_boxplot() +
...
You will need to extract your ordination scores from your ordination output; here we will again look at the species scores.  Unfortunately the default return from the `scores` command is a matrix and `ggplot` expects a data.frame (data.frames and tibbles are more versatile for plotting)

```{r extract spp scores for ggplot}
dunespp.sco <- data.frame(scores(dune.pca, display="species"))
```

We can use ggplot in the usual way, building up layer by layer, but take advantage of extra commands provided by `ggrepel` to reduce overlap between labels:

```{r ggplot and ggrepel of dune.pca}
ggplot(dunespp.sco, aes(x=PC1, y=PC2, label=rownames(dunespp.sco))) +
   geom_point() +
   geom_text_repel() +
   theme_classic()
```
...
# Convert into a tibble, changing rownames to a new column SppID
dunespp.sco <- as_tibble(rownames_to_column(data.frame(dunespp.sco), var="SppID"))

ggplot(filter(dunespp.sco, SppID != "Empenigr"), aes(x=PC1, y=PC2, label=SppID)) +
   geom_point() +
   geom_text_repel() +
   theme_classic()
```

If you wanted to drop both _Empetrum nigrum_ and _Rumex acetosa_ it is usually easiest to have them in a list and precede SppID with `!` (for "NOT") and use the `%in%` operator:

```{r drop 2 spp from plot, warning=FALSE}
ggplot(filter(dunespp.sco, !SppID %in% c("Empenigr", "Rumeacet")),
       aes(x=PC1, y=PC2, label=SppID)) +
   geom_point() +
   geom_text_repel() +
   theme_classic()
```
...
dunespp.sco <- scores(dune.pca, display="species")
write.csv(dunespp.sco, "dunespp.csv")  # Use write.csv rather than write_csv
```

If you go to your NES8010 folder in File Explorer you should see the file `dunespp.csv` listed; depending on your Microsoft Windows configuration, it might only be listed as `dunespp`.  Double-click on it and by default it will open in Microsoft Excel.  After the data are loaded, you need to insert a chart.
